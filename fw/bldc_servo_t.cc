...
class BldcServo::Impl {
 ...
 private:
  ...
  void ISR_DoPositionCommon(
	...
	const float pace_vel = velocity_command * 2.0f * kPi;
	const float pace_acceleration = status_.control_acceleration * 2.0f * kPi;

	const float pace_inertia_Nm  = config_.pace_inertia * pace_acceleration;
    const float pace_damping_Nm  = config_.pace_damping * pace_vel;

    const float pace_coulomb_Nm = [&]() MOTEUS_CCM_ATTRIBUTE {
      if (config_.pace_coulomb_friction == 0.0f) { return 0.0f; }
      if (pace_vel >  config_.pace_deadband) { return  config_.pace_coulomb_friction; }
      if (pace_vel < -config_.pace_deadband) { return -config_.pace_coulomb_friction; }
      if (config_.pace_deadband > 0.0f) {
        return config_.pace_coulomb_friction * (pace_vel / config_.pace_deadband);
      }
      return 0.0f;
    }();
    ...
    const float unlimited_torque_Nm =
        (pid_position_.Apply(
            ...
            pid_options) +
         feedforward_Nm +
         inertia_feedforward_Nm + pace_inertia_Nm + pace_damping_Nm + pace_coulomb_Nm);
